import gc
import logging
from utils.dataset import TextVideoPairDataset, cycle
from model import MMRegression, MMDiffusion
...
class Trainer:
    def __init__(self, config):
        ...
        assert config.distribution_loss == "mm", "Only ODE loss is supported for ODE training"
        if config.model_loss == "diffusion":
            self.model = MMDiffusion(config, device=self.device)
        else:
            self.model = MMRegression(config, device=self.device)
        ...
        #     print("Generator optimized parameters:")
        #     for name, param in self.model.generator.named_parameters():
        #         if param.requires_grad:
        #             print(f" - {name}: {param.shape}")

        # Step 3: Initialize the dataloader
        resize_480p= transforms.Compose([
            transforms.Resize((480, 832)),
            transforms.ToTensor(),
        ])

        dataset = TextVideoPairDataset(config.data_path, num_frames=config.num_frames, transform=resize_480p)
    ...
    def train_one_step(self):
        ...
        source_pixel = batch["source_video"].to(device=self.device).permute(0, 4, 1, 2, 3).float()  # [B, C, T, H, W]
        edited_pixel = batch["edited_video"].to(device=self.device).permute(0, 4, 1, 2, 3).float()  # [B, C, T, H, W]
        text_prompts = batch["prompts"]

        # Step 2: Extract the conditional infos
        with torch.no_grad():
            # print("Encoding videos to latents...")
            # print(f"source_pixel shape: {source_pixel.shape}, edited_pixel shape: {edited_pixel.shape}")
            self.model.vae.to(device=self.device)
            source_latent = self.model.vae.encode_to_latent(source_pixel) # [B, C', T, H', W']
            target_latent = self.model.vae.encode_to_latent(edited_pixel) # [B, C', T, H', W']
            conditional_dict = self.model.text_encoder(
                text_prompts=text_prompts)
            conditional_dict["source_latent"] = source_latent
        ...
        if VISUALIZE and not self.config.no_visualize and not self.config.disable_wandb and self.is_main_process:
            ...
            # 将 Latent 解码回像素进行对比
            source_vid_out = source_pixel # 直接用原像素
            target_vid_out = edited_pixel # 直接用原像素
            pred_vid_out = self.model.vae.decode_to_pixel(output)

            input_video = 255.0 * (source_vid_out.cpu().numpy() * 0.5 + 0.5)
            output_video = 255.0 * (pred_vid_out.cpu().numpy() * 0.5 + 0.5)
            ground_truth_video = 255.0 * (target_vid_out.cpu().numpy() * 0.5 + 0.5)
