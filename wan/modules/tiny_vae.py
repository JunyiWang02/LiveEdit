...
class TAEHV(nn.Module):
    ...
    def encode_to_latent(self, x, parallel=True, show_progress_bar=True):
        """Encode a sequence of frames.

        Args:
            x: input NCTHW RGB (C=3) tensor with values in [0, 1].
            parallel: if True, all frames will be processed at once.
              (this is faster but may require more memory).
              if False, frames will be processed sequentially.
        Returns NTCHW latent tensor with ~Gaussian values.
        """
        x = x.permute(0, 2, 1, 3, 4) # NCTHW -> NTCHW
    ...
    def decode_to_pixel(self, latent: torch.Tensor, use_cache: bool = False) -> torch.Tensor:
        # input [batch_size, num_frames, num_channels, height, width]
        # output [batch_size, num_frames, num_channels, height, width]
        return self.decode_video(latent, parallel=False)
