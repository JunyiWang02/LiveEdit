...
class MMInferencePipeline(torch.nn.Module):
    def __init__(
            self,
            args,
            device,
        ...
        self.generator = WanDiffusionWrapper(
            **getattr(args, "model_kwargs", {}), is_causal=True, local_attn_size=local_attn_size) if generator is None else generator
        ...
        elif hasattr(args, "inference_num_steps"):
    ...
    def inference(
        ...
        """
        print(f"[MMInferencePipeline] {y is not None=}")
        ...
        if initial_latent is not None:
            # print("[DEBUG] Caching context features from initial latent...")
        ...
        for frame_idx, current_num_frames in enumerate(all_num_frames):
            ...
            # 只采一次全局噪声，整个采样过程固定它
            noise0 = torch.randn_like(noisy_input)
            y_input = None
            if y is not None:
                y_input = y[
                    :, current_start_frame - num_input_frames:current_start_frame + current_num_frames - num_input_frames]
            ...
            for index, current_timestep in enumerate(self.denoising_step_list):
                ...
                timestep = torch.ones(
                    ...
                    device=noise.device,
                    dtype=noise.dtype) * current_timestep
                ...
                print(f"{index=}, {int(current_timestep)}, {float(noisy_input.std())} {float(flow_pred.std())}, {float(x0_pred.std())}")


                if index < len(self.denoising_step_list) - 1:
                    sigma_next = self.scheduler.sigmas[index + 1]
                    noisy_input = (1 - sigma_next) * x0_pred + sigma_next * noise0

                    # # ✅ 正确：用 flow_pred 做 ODE/flow 的一步更新
                    # noisy_input = self.scheduler.step(
                    #     model_output=flow_pred.flatten(0, 1),      # [B*T,C,H,W]
                    #     timestep=timestep.flatten(0, 1),           # [B*T]
                    #     sample=noisy_input.flatten(0, 1)           # [B*T,C,H,W]
                    # ).unflatten(0, (batch_size, current_num_frames)).to(dtype=noise.dtype)
                else:
                    denoised_pred = x0_pred  # 最后一步输出你要的 x0
        ...
        if profile:
            ...
            vae_start.record()
        # if "pooled_attn_rows" in debug_dict:
        #     import matplotlib.pyplot as plt
        #     rows = debug_dict["pooled_attn_rows"]  # list of [N]
        #     mat = torch.stack(rows, dim=0).numpy() # [L, N]
        #     plt.figure(figsize=(10, 8))
        #     plt.imshow(mat, aspect="auto")
        #     plt.xlabel("Key/Value Tokens (History) - pooled x1560 (chunk index)")
        #     plt.ylabel("Query Tokens (All Blocks) - pooled x1560 (block index)")
        #     plt.colorbar()
        #     plt.tight_layout()
        #     plt.savefig("frame40_step_0_pooled.png", dpi=200)
